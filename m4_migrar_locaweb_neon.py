...
"""
Migração Direta: Locaweb -> Neon (DB-to-DB)
- Origem (Source): Locaweb (DATABASE_URL)
- Destino (Target): NeonDB (DATABASE_URL_NEON)
- NÃO sobrescreve dados do destino (dedup por chave)
- Normaliza textos, preenche defaults (timestamps/booleans)
- Ordem hierárquica respeitada
- Sincroniza Sequences (IDs) automaticamente no final da migração
...
MANUAL_PRIORITY = [
    ...
    "vendas",
    "itens_venda",
    "compra_nf",       # Adicionado (Pai do estoque/itens)
    "compra_item",     # Adicionado
    "estoque_itens",   # Adicionado
    "certidoes",       # Adicionado
    "produto_historico",# Adicionado (Geralmente a maior tabela)
    "pedido_compra",
    "item_pedido",
    "notificacao",
    "comunicacoes",
...
def load_env_or_die():
    load_dotenv()
    # INVERTIDO: Origem agora é a Locaweb, Destino é a Neon
    url_src = os.getenv("DATABASE_URL") 
    url_dst = os.getenv("DATABASE_URL_NEON")
    
    if not url_src or not url_dst:
        print("ERRO: defina DATABASE_URL (Locaweb) e DATABASE_URL_NEON (Neon) no .env", file=sys.stderr)
        sys.exit(1)
    return url_src, url_dst

def create_engines():
    url_src, url_dst = load_env_or_die()
    # Força apenas 1 conexão com a Locaweb para evitar o erro FATAL
    eng_src = create_engine(url_src, pool_size=1, max_overflow=0)
...
def resetar_sequences_neon(engine: Engine, meta: MetaData):
    """Sincroniza os contadores de ID no PostgreSQL (Sequences) para evitar erros de Primary Key"""
    print("\n>> Sincronizando IDs automáticos (Sequences) na NeonDB...")
    with engine.begin() as conn:
...
# ============== Main ==============
def main():
    parser = argparse.ArgumentParser(description="Migrar dados diretamente da Locaweb para Neon.")
    ...
    print(">> Iniciando migração direta DB-to-DB: Locaweb -> Neon")
    ...
    for tname in order:
        ...
        print(f"\n[Tabela {tname}] Transferindo dados... | Chave: {key_desc}")
                ...
                try:
                    ...
                    if ident in existing:
                        ...
                        if len(skipped_log) < 2000:
                            skipped_log.append([safe_json_dumps(row)])
                        continue
                    ...
                    if len(errors_log) < 2000:
                        errors_log.append([str(e), safe_json_dumps(raw)])
                    continue
    ...
    resetar_sequences_neon(eng_dst, meta_dst)

    print("\n✅ Migração concluída com sucesso.")
    print("Logs guardados em:", log_dir)
